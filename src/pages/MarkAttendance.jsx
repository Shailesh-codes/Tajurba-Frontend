...
const MarkAttendance = () => {
  ...
  const handleContinue = () => {
    ...
    if (markingType === 'attendance') {
      navigate(`/attendance-venue-fee?marking_type=${markingType}&type=${attendanceType}&meeting_id=${meetingId}`);
    } else {
      navigate(`/mark-venue-fee?type=${attendanceType}&meeting_id=${meetingId}`);
    }
  };
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div className="flex gap-3">
          ...
          <div className="flex flex-col">
            <h1 className="text-2xl font-bold text-white">Mark Meeting Attendance</h1>
            <h2 className="text-sm text-gray-400">Record attendance and venue fees for meetings</h2>
          </div>
        </div>
      </div>
      ...
      <div className="bg-gray-800 rounded-xl border border-gray-700 p-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
          ...
          <div className="bg-gray-800/50 rounded-xl p-4 border border-gray-700">
            <div className="flex items-center justify-between mb-3">
              <div>
                ...
                <h3 className="text-2xl font-bold text-white mt-1">{stats.meetings.total}</h3>
              </div>
            ...
            <div className="flex items-center gap-3">
              <div className="px-2.5 py-1 bg-green-500/10 rounded-lg">
                <span className="text-sm text-green-500">{stats.meetings.marked} Marked</span>
              </div>
              <div className="px-2.5 py-1 bg-orange-500/10 rounded-lg">
                <span className="text-sm text-orange-500">{stats.meetings.pending} Pending</span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-gray-800/50 rounded-xl p-4 border border-gray-700">
            ...
            <div className="flex items-center gap-3">
              <div className="px-2.5 py-1 bg-green-500/10 rounded-lg">
                <span className="text-sm text-green-500">{stats.mdp.marked} Marked</span>
              </div>
              <div className="px-2.5 py-1 bg-orange-500/10 rounded-lg">
                <span className="text-sm text-orange-500">{stats.mdp.pending} Pending</span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-gray-800/50 rounded-xl p-4 border border-gray-700">
            <div className="flex items-center justify-between mb-3">
              <div>
                ...
                <h3 className="text-2xl font-bold text-white mt-1">{stats.social.total}</h3>
              </div>
            ...
            <div className="flex items-center gap-3">
              <div className="px-2.5 py-1 bg-green-500/10 rounded-lg">
                <span className="text-sm text-green-500">{stats.social.marked} Marked</span>
              </div>
              <div className="px-2.5 py-1 bg-orange-500/10 rounded-lg">
                <span className="text-sm text-orange-500">{stats.social.pending} Pending</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-4 mb-8">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <select
              ...
              onChange={(e) => setFormData({ ...formData, markingType: e.target.value })}
              className="w-full bg-gray-800 text-white p-3 rounded-xl border border-gray-700 focus:border-green-500 focus:ring-0"
            >
            ...
            >
              <option value="">
                {!formData.attendanceType
                  ? "Select Meeting"
                  : `Select ${formData.attendanceType === "mdp" ? "MDP" : formData.attendanceType === "social" ? "Social & Training" : "Meeting"}`}
              </option>
          ...
          <div className="flex justify-end mt-4">
            <button
              ...
              className="px-6 py-3 bg-gradient-to-r from-green-600 to-green-900 hover:from-green-700 hover:to-green-950 text-white font-medium rounded-xl flex items-center justify-center gap-2 transition-all duration-300 shadow-lg hover:shadow-xl hover:shadow-green-900/30 hover:-translate-y-0.5"
            >
              <span>
